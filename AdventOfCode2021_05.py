...
def compareValues(PointA, PointB, Direction):
    if Direction is 'Horizontal':
        ...
        if int(PointA[0]) > int(PointB[0]) and int(PointA[1]) < int(PointB[1]):
            ...
            newPointA = [int(PointA[0]), int(PointA[1])]
            newPointB = [int(PointB[0]), int(PointB[1])]
    new_point_order = [newPointA, newPointB]
    return new_point_order


def findFormula(PointA, PointB):
    Horizontal_Vertical = 'None'

    if (int(PointB[1]) - int(PointA[1])) == 0 or (int(PointB[0]) - int(PointA[0])) == 0:
        m = 0
        if int(PointB[1]) - int(PointA[1]) == 0:
            Horizontal_Vertical = 'Horizontal'
            b = int(PointA[1])
            formula = 'y = ' + str(b)
            point_order = compareValues(PointA, PointB, Horizontal_Vertical)
        elif int(PointB[0]) - int(PointA[0]) == 0:
        ...
        point_order = compareValues(PointA, PointB, Horizontal_Vertical)
        m = (int(PointB[1]) - int(PointA[1])) / (int(PointB[0]) - int(PointA[0]))
        b = -(int(PointA[0]) * m) + (int(PointA[1]))
        formula = 'y = ' + str(m) + 'x + ' + str(b)
        # print formula
    return [Horizontal_Vertical, point_order[0], point_order[1], m]
...
for i in p_input:
    ...
    segment = findFormula(point1, point2)
    if segment[0] is not 'None':
        lineInfo = [segment[1], segment[2], segment[0]]
        segment_list.append(lineInfo)
        slope_list.append(segment[-1])
total_points_list = []
slope = 0
for s in segment_list:
    if s[-1] is 'Horizontal':
        temp = s[0][0]
        endPoint = s[-2][0]
        constant = s[0][1]
        temp = temp + 1
        while temp < endPoint:
            insert_point = [temp, constant]
            s.insert(-2, insert_point)
            temp = temp + 1
    elif s[-1] is 'Vertical':
        temp = s[0][1]
        endPoint = s[-2][1]
        constant = s[0][0]
        temp = temp + 1
        while temp < endPoint:
            insert_point = [constant, temp]
            s.insert(-2, insert_point)
            temp = temp + 1
    # Part 2 : Comment Out for Part 1
    elif s[-1] is 'Diagonal':
        if slope_list[slope] > 0:
            tempX = s[0][0]
            tempY = s[0][1]
            endPoint = s[-2][0]
            tempX = tempX + 1
            tempY = tempY + 1
            while tempX < endPoint:
                insert_point = [tempX, tempY]
                s.insert(-2, insert_point)
                tempX = tempX + 1
                tempY = tempY + 1
        elif slope_list[slope] < 0:
            tempX = s[0][0]
            tempY = s[0][1]
            endPoint = s[-2][0]
            tempX = tempX + 1
            ...
            while tempX < endPoint:
                ...
                tempX = tempX + 1
                tempY = tempY - 1
    total_points_list = total_points_list + s[:-1]
    slope = slope + 1
total_points_check = []
duplicate_points = []
count_list = []
for point in total_points_list:
    string_point = '(' + str(point[0]) + ', ' + str(point[1]) + ')'
    total_points_check.append(string_point)
single_points = list(set(total_points_check))
for element in range(len(total_points_check)):
    print total_points_check[element]
    if total_points_check.count(total_points_check[element]) > 1:
        duplicate_points.append(total_points_check[element])
print len(list(set(duplicate_points)))