...
for i in range(len(p_cards)):
    ...
    for column in range(len(card)):
        ...
        for c in card:
            c = c.split(' ')
            column_set.insert(0, c[column])
        column_set.insert(0, 'CardNumber:' + str(i))
        column_set.append('Tally:0')
        bingo_options_total.append(column_set)
    for c in card:
        c = c.split(' ')
        c.insert(0, 'CardNumber:' + str(i))
        c.append('Tally:0')
        bingo_options_total.append(c)
print bingo_options_total

for p in range(len(p_input)):
    for bingo_options in bingo_options_total:
        bingo_options_check = bingo_options[1:-1]
        if p_input[p] in bingo_options_check:
            tally = bingo_options[-1]
            tally = int(tally[-1])
            tally = str(tally + 1)
            bingo_options[-1] = 'Tally:' + tally
            if int(tally) == bingo_length:
                number_passes = p
                winner_card = bingo_options[0]
                winner_card = winner_card[11:]
                winner_card = int(winner_card)
                winner_number = int(p_input[p])
                winner = True
                print 'WINNER', bingo_options[0], bingo_options[1:-1]
                print 'Number:' + str(p_input[p]), 'Passes:' + str(number_passes)
                break
    if winner is True:
        break
print cardValue(p_input, p_cards, winner_card, number_passes) * winner_number
...
for p in range(len(p_input)):
    for bingo_options in bingo_options_total:
        ...
        if p_input[p] in bingo_options_check:
            ...
            tally = str(tally + 1)
            bingo_options[-1] = 'Tally:' + tally
            if len(bingo_options_total) <= 10:
                loser_card = bingo_options[0]
                loser_card = loser_card[11:]
                loser_card = int(loser_card)
                print 'LOSER', bingo_options[0], bingo_options[1:-1]
            if int(tally) == bingo_length:
                number_passes = p
                winner_card = bingo_options[0]
                winner_card = winner_card[11:]
                ...
                print 'WINNER', bingo_options[0], bingo_options[1:-1]
                print 'Number:' + str(p_input[p]), 'Passes:' + str(number_passes)
                winner_card_number = bingo_options[0]
            if winner is True:
                bingo_options_total = removeItemsFromList(bingo_options_total, winner_card_number)
                winner = False
    if losing is True:
        break
print cardValue(p_input, p_cards, loser_card, number_passes) * winner_number